...
def edge_mask(bs):
    #tensor_shape = tens.shape[1:]
    array = np.ones([bs]+[80, 80, 3])
    #tensor_shape = tens.shape
    #array = np.ones(tensor_shape)
    array[:, :3, :, :] = 0.
    array[:, :, :3, :] = 0.
    array[:, -3:, :, :] = 0.
    array[:, :, -3:, :] = 0.
    #print('nparray:', array, array.shape, array[0][2][2], array[0][3][3])
    tensor = tf.convert_to_tensor(array, dtype=tf.float32)
    #print('tensor for out:', tensor, tensor.shape, tensor[0])
    return tensor
