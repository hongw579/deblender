...
from utils_data import np_to_tfrecords
...
class RDN(object):
   ...
   def train(self, config):
    ...
    def load_sess_rdn(self, config):

        def norm_by_channel(in_array):
            for idx in range(len(in_array)):
                in_array[idx, :, :, 0] = (in_array[idx, :, :, 0]-in_array[idx, :, :, 0].min())/(in_array[idx, :, :, 0].max()-in_array[idx, :, :, 0].min())
                in_array[idx, :, :, 1] = (in_array[idx, :, :, 1]-in_array[idx, :, :, 1].min())/(in_array[idx, :, :, 1].max()-in_array[idx, :, :, 1].min())
                in_array[idx, :, :, 2] = (in_array[idx, :, :, 2]-in_array[idx, :, :, 2].min())/(in_array[idx, :, :, 2].max()-in_array[idx, :, :, 2].min())
            return in_array
