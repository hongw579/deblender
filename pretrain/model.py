...
class RDN(object):
    ...
    def train(self, config):
        ...
        self.images, self.labels, self.images2 = self.data.get_batch()
        ...
        for i_gpu in range(self.n_gpus):
            with tf.device("/gpu:%d" % i_gpu):
                _x = self.images[i_gpu*self.batch_size: (i_gpu+1)*self.batch_size]
                _y = self.labels[i_gpu*self.batch_size: (i_gpu+1)*self.batch_size]
                i_pred = self.model(_x)
                i_loss = tf.reduce_mean(tf.abs(_y - i_pred))
                i_grads = optimizer.compute_gradients(i_loss)
                tower_grads.append(i_grads)
                tower_loss.append(i_loss)

        grads = average_gradients(tower_grads)
        learning_step = optimizer.apply_gradients(grads)

        total_loss = tf.stack(tower_loss)
        self.loss = tf.reduce_mean(total_loss, 0) 

        self.summary = tf.summary.scalar('loss', self.loss)
        self.summary_image = tf.summary.image("Image_blended", self.images)
        self.summary_label = tf.summary.image("Image_X1", self.labels)
        ...
        for ep in range(epoch_start, config.epoch):
            ...
            for idx in range(batch_start, batch_num):
                ...
                _, err, lr = self.sess.run([learning_step, self.loss, learning_rate])
